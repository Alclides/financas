...
@section('content')
    <div class="container my-5">
        ...
        <form action="{{ isset($transacao) ? route('transacoes.update', $transacao->id) : route('transacoes.store') }}"
            method="POST" class="p-5 border rounded-lg shadow bg-white">
            @csrf
            @if (isset($transacao))
                @method('PUT')
            @endif
            ...
            <div class="form-group mb-4">
                ...
                <input type="number" class="form-control form-control-lg" id="valor" name="valor" step="0.01"
                    value="{{ old('valor', $transacao->valor ?? '') }}" required placeholder="Digite o valor da transação">
            </div>
                ...
                <select class="form-control form-control-lg" id="categoria_id" name="categoria_id" required>
                    ...
                    @foreach ($categorias as $categoria)
                        <option value="{{ $categoria->id }}"
                            {{ old('categoria_id', $transacao->categoria_id ?? '') == $categoria->id ? 'selected' : '' }}>
                            {{ $categoria->nome }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="form-group mb-4">
                ...
                <input type="date" class="form-control form-control-lg" id="data" name="data"
                    value="{{ old('data', isset($transacao) ? \Carbon\Carbon::parse($transacao->data)->format('Y-m-d') : '') }}"
                    required placeholder="Selecione a data da transação">
            </div>
